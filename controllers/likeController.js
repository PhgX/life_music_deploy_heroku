...
const LikeController = {
    ...
    try {
      ...
      if (like) {
        if (index === -1) {
          ...
          let success = await Like.findByIdAndUpdate(
            { _id: mongoose.Types.ObjectId(likeId) },
            { $push: { user: userId } },
            { upsert: true, new: true }
          );
          ...
          res
            .status(200)
            .json({ msg: "like bài hát thành công", likeNumber, like, song });
        } else if (index !== -1) {
        ...
        if (index !== -1) {
          ...
          like = false;
          res.status(200).json({ msg: "unlike thành công", likeNumber, like, song });
        } else if (index === -1) {
  ...
  //likeRouter.get('/song/isliked/:id', auth, likeController.checkSongIsLiked);
  checkSongIsLiked: async (req, res) => {
    let songId = req.params.id;
    let userId = req.body.user;
    let check = await User.find({ likeSongs : { "$elemMatch" : {$eq: `${songId}`}}}, {_id : userId}).exec();
